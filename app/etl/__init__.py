...
class ETLPipeline:
    ...
    def __init__(self):
        ...
        self._acquisition_controllers: Dict[str, Any] = {}  # country -> controller
        self._acquisition_metrics: Dict[str, Any] = {}      # aggregated metrics
    ...
    def run(
        ...
        if use_acquisition:
            ...
            validated_jobs = self._run_adaptive_acquisition(countries, metrics)
        else:
            ...
            validated_jobs = self._process_jobs(raw_jobs, metrics)

        # ------------------------------------------------------------
        # Phase 5: Load (common for both paths)
        # ------------------------------------------------------------
        if validated_jobs:
            metrics = self._load_jobs(validated_jobs, metrics)
        else:
        ...
        try:
            for country in countries:
                ...
                logger.info(
                    ...
                    f"tech={progress['db_tech']}, non_tech={progress['db_non_tech']}, "
                ...
                while batch_number < max_batches:
                    ...
                    logger.info(f"Processing {len(batch_raw_jobs)} jobs (batch limit: {batch_limit})")

                    # 4. Add to controller (tracking intent)
                    added, duplicates = controller.add_jobs(batch_raw_jobs, query, country)
                    ...
                    logger.info(
                        ...
                        f"tech_classified={progress['tech_classified_acquired']}, "
                        f"remaining_needed={progress['remaining_tech_needed']}, "
                        f"projected_has_parity={progress['projected_has_parity']}"
                    )
                ...
                logger.info(
                    ...
                    f"  Non-tech classified: {result.non_tech_classified_count}\n"
                    f"  Unclassified: {result.unclassified_count}\n"
                    f"  Actual tech ratio: {result.actual_classified_tech_ratio:.2%}\n"
    ...
    def _aggregate_acquisition_metrics(self) -> Dict[str, Any]:
        ...
        actual_tech_ratio = total_tech_classified / total_classified if total_classified > 0 else 0.0
    ...
    def _extract_legacy(self, countries: List[str]) -> List[Dict]:
        ...
        for country in countries:
            try:
                ...
                logger.info("Extracted %d jobs from %s", len(country_jobs), country)
    ...
    def _process_jobs(self, raw_jobs: List[Dict], metrics: PipelineMetrics) -> List[JobValidated]:
    ...
    def get_acquisition_metrics(self) -> Optional[Dict[str, Any]]:
    ...
    def get_acquisition_controllers(self) -> Dict[str, Any]:
