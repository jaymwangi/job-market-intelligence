...
class ThresholdOverride:
    ...
    def apply_to(self, base: EffectiveThresholds) -> EffectiveThresholds:
        ...
        return EffectiveThresholds(
            ...
            minimum_margin=(
                self.minimum_margin if self.minimum_margin is not None else base.minimum_margin
            ),
            min_confidence=(
                self.min_confidence if self.min_confidence is not None else base.min_confidence
            ),
        )
...
class ClassificationPolicy:
    ...
    min_confidence: float | None = 0.15
    ...
    def _parse_overrides(overrides: dict[str, Any]) -> dict[str, ThresholdOverride]:
        ...
        for category_id, override_data in overrides.items():
            if isinstance(override_data, ThresholdOverride):
                ...
                raise ValueError(f"Invalid override type for {category_id}: {type(override_data)}")
        return result
