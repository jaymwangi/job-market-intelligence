...
from .models import AcquisitionMode, AcquisitionResult, AcquisitionStats, DatabaseComposition
...
class AcquisitionController:
    ...
    def __init__(
        ...
        tech_queries: list[str] | None = None,
        ...
        self.broad_queries = self._build_queries(
            broad_queries or self._get_default_broad_queries(), is_tech=False
        )
        self.tech_queries = self._build_queries(
            tech_queries or self._get_default_tech_queries(), is_tech=True
        )

        # Query tracking
        self.broad_index: int = 0
        self.tech_index: int = 0
        self.used_query_keys: set[str] = set()
        self.broad_queries_used: int = 0
        self.tech_queries_used: int = 0
        ...
        self.tech_intent_jobs: list[dict[str, Any]] = []
        self.broad_intent_jobs: list[dict[str, Any]] = []

        # CLASSIFICATION-based collections
        self.tech_classified_jobs: list[dict[str, Any]] = []
        self.non_tech_classified_jobs: list[dict[str, Any]] = []
        self.unclassified_jobs: list[dict[str, Any]] = []
        ...
        self.current_query_start_time: datetime | None = None
        self.current_query_new_jobs: int = 0
        self.current_query_classified: int = 0
        self.current_query_tech: int = 0
        self.current_query_non_tech: int = 0
        self.current_query_unclassified: int = 0
        self.current_query_mode: AcquisitionMode | None = None  # Track mode at query start
        ...
        self.balanced_mode_tech_start: int = 0
        self.balanced_mode_broad_start: int = 0
    ...
    def _get_database_composition(self, country: str | None = None) -> DatabaseComposition:
    ...
    def _determine_mode(self) -> AcquisitionMode:
        ...
        if comp.has_reached_parity(self.parity_tolerance):
            logger.info("Parity reached, starting in BALANCED mode")
            return AcquisitionMode.BALANCED
    ...
    def get_next_query(self) -> dict[str, Any] | None:
        """
    ...
    def _get_catch_up_query(self, stats: AcquisitionStats) -> dict[str, Any] | None:
        """
    ...
    def _get_balanced_query(self, stats: AcquisitionStats) -> dict[str, Any] | None:
        """
    ...
    def _next_broad_query(self) -> dict[str, Any] | None:
    ...
    def _next_tech_query(self) -> dict[str, Any] | None:
    ...
    def add_jobs(
        self, jobs: list[dict[str, Any]], query: dict[str, Any], country: str
    ) -> tuple[int, int]:
        new_count = 0
        dup_count = 0
    ...
    def _record_query_metrics(self) -> None:
        ...
        if self.current_query_start_time:
            ...
            is_tech_intent = (
                self.current_query.get("category") == "it-jobs" if self.current_query else False
            )
            ...
            self.query_metrics.append(
                {
                    ...
                    "mode": (
                        self.current_query_mode.value
                        if self.current_query_mode
    ...
    def update_classification(self, classified_jobs: list[JobEnriched]) -> None:
        """
        ...
        acquired_job_ids = {self._get_job_id(job) for job in self.all_jobs}
        ...
        for job in classified_jobs:
            ...
            if is_tech is True:
                tech_count += 1
                self.tech_classified_jobs.append(job.model_dump())
                self.current_query_tech += 1

            elif is_tech is False:
                non_tech_count += 1
                self.non_tech_classified_jobs.append(job.model_dump())
                self.current_query_non_tech += 1

            else:
                unclassified_count += 1  # type: ignore
                self.unclassified_jobs.append(job.model_dump())
        ...
        self.projected_composition.total_jobs += tech_count + non_tech_count + unclassified_count
        ...
        if self.mode == AcquisitionMode.CATCH_UP and self.projected_composition.has_reached_parity(
            self.parity_tolerance
        ):
            logger.info("🎯 Projected composition reached parity! " "Switching to balanced mode")
    ...
    def get_result(self) -> AcquisitionResult:
        ...
        for job in self.all_jobs:
            if isinstance(job, dict):
                ...
                is_dup = getattr(job, "_is_duplicate", False)  # type: ignore
    ...
    def get_progress(self) -> dict[str, Any]:
        ...
        return {
            ...
            "projected_has_parity": self.projected_composition.has_reached_parity(
                self.parity_tolerance
            ),
