...
class AnalyticsRepository:
    ...
    def get_salary_statistics(self) -> Dict[str, Any]:
        ...
        if salaries:
            mid = len(salaries) // 2
            median = salaries[mid][0] if len(salaries) % 2 == 1 else (salaries[mid-1][0] + salaries[mid][0]) / 2
    ...
    def get_employment_type_distribution(self) -> List[Dict[str, Any]]:
        ...
        return [{"employment_type": r.employment_type or "Unknown", "count": r.count} for r in results]
    ...
    def get_distinct_source_sites(self) -> List[str]:
        ...
        results = self.db.query(Job.source_site).filter(
            Job.is_active == True,
            Job.is_deleted == False
        ).distinct().all()
    ...
    def get_dataset_summary(self) -> Dict[str, Any]:
        ...
        unique_companies = self.db.query(Job.company_name).filter(
            Job.company_name.isnot(None),
            Job.is_active == True,
            Job.is_deleted == False
        ).distinct().count()
        
        unique_locations = self.db.query(Job.location).filter(
            Job.location.isnot(None),
            Job.is_active == True,
            Job.is_deleted == False
        ).distinct().count()
