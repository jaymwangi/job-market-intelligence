...
def print_recent_jobs(jobs, days):
    ...
    for job in jobs[:10]:
        ...
        location = (
            job.location[:17] + "..."
            if job.location and len(job.location) > 20
            else job.location or "Unknown"
        )
        print(f"{title:<40} {company:<20} {location:<20}")
...
def print_dataset_summary(repo):
    ...
    print("\n📋 Field Coverage:")
    print(
        f"  Company name     : {jobs_with_company:>5,} / {total_jobs:,} ({pct(jobs_with_company):>5.1f}%)"
    )
    print(
        f"  Location         : {jobs_with_location:>5,} / {total_jobs:,} ({pct(jobs_with_location):>5.1f}%)"
    )
    print(
        f"  Salary data      : {jobs_with_salary:>5,} / {total_jobs:,} ({pct(jobs_with_salary):>5.1f}%)"
    )
    print(
        f"  Employment type  : {jobs_with_employment:>5,} / {total_jobs:,} ({pct(jobs_with_employment):>5.1f}%)"
    )
    print(
        f"  Posted date      : {jobs_with_date:>5,} / {total_jobs:,} ({pct(jobs_with_date):>5.1f}%)"
    )
