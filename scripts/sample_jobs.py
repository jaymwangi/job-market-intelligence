...
class JobMetadata:
    ...
    def to_dict(self, include_description: bool = True) -> dict[str, Any]:
        ...
        row = {
            ...
            "posted_date": (
                self.job.posted_date.strftime("%Y-%m-%d") if self.job.posted_date else ""
            ),
...
def fill_remaining(
    ...
    if remaining:
        additional = rng.sample(remaining, min(target_count - len(sampled), len(remaining)))
...
def _allocate_largest_remainder(
    ...
    if seats_left > 0:
        sorted_by_size = sorted(groups.items(), key=lambda x: len(x[1]), reverse=True)
...
def build_metadata(
    ...
    for i, job in enumerate(jobs_to_score):
        ...
        try:
            ...
            result = scorer.score(job.title or "", job.description or "", skill_names)
...
def sample_by_country_from_metadata(
    ...
    allocations = _allocate_largest_remainder(country_groups, total, count, min_allocation)
...
def sample_by_ambiguity_from_metadata(
    ...
    logger.info(
        f"  High ambiguity: {len(ambiguity_groups['high'])}, "
    ...
    allocations = _allocate_ambiguity_proportional(ambiguity_groups, total, count)
...
def sample_hybrid_from_metadata(
    ...
    """
    logger.info(
        "  Running hybrid sampling (30% random + 30% ambiguous + 20% tech + 20% non-tech)..."
    )
    ...
    logger.info(
        f"  Random: {random_count}, Ambiguous: {ambiguous_count}, "
    ...
    if ambiguous_count > 0:
        ...
        if high_ambiguity:
            ambiguity_sample = rng.sample(high_ambiguity, min(ambiguous_count, len(high_ambiguity)))
    ...
    if non_tech_count > 0:
        ...
        if non_tech_metadata:
            non_tech_sample = rng.sample(
                non_tech_metadata, min(non_tech_count, len(non_tech_metadata))
            )
...
def validate_args(args: argparse.Namespace) -> bool:
    ...
    if args.count > args.limit:
        logger.warning(
            f"Count ({args.count}) exceeds limit ({args.limit}). " "Consider increasing --limit"
        )
...
def main():
    ...
    with get_db_session() as session:
        ...
        logger.info(
            f"Unique jobs: {len(unique_jobs)} (removed {len(jobs) - len(unique_jobs)} duplicates)"
        )
