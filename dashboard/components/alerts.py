...
def show_error(message: str, role: str = "alert") -> None:
    ...
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:0.5rem;color:#721c24;background:#f8d7da;border:1px solid #f5c6cb;border-radius:8px;padding:0.75rem 1rem;"><span style="display:inline-flex;">{icon}</span> <span>{message}</span></div>',
...
def show_success(message: str, role: str = "status") -> None:
    ...
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:0.5rem;color:#155724;background:#d4edda;border:1px solid #c3e6cb;border-radius:8px;padding:0.75rem 1rem;"><span style="display:inline-flex;">{icon}</span> <span>{message}</span></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div role="{role}" aria-label="Success: {message}"></div>', unsafe_allow_html=True
    )
...
def show_warning(message: str, role: str = "alert") -> None:
    ...
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:0.5rem;color:#856404;background:#fff3cd;border:1px solid #ffeeba;border-radius:8px;padding:0.75rem 1rem;"><span style="display:inline-flex;">{icon}</span> <span>{message}</span></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div role="{role}" aria-label="Warning: {message}"></div>', unsafe_allow_html=True
    )
...
def show_info(message: str, role: str = "status") -> None:
    ...
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:0.5rem;color:#0c5460;background:#d1ecf1;border:1px solid #bee5eb;border-radius:8px;padding:0.75rem 1rem;"><span style="display:inline-flex;">{icon}</span> <span>{message}</span></div>',
        unsafe_allow_html=True,
    )
