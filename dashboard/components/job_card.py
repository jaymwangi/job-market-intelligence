...
def render_job_card(job) -> None:
    ...
    company = _escape_html(
        job_dict.get("company_name") or job_dict.get("company", "Unknown Company")
    )
    ...
    tags = [f'<span class="job-card-tag job-card-tag-country">{flag} {location_display}</span>']

    # Language badge (Sprint 6.6)
    if not is_english:
        tags.append(
            f'<span class="job-card-tag job-card-tag-language">{language_emoji} {language_label}</span>'
        )
    else:
        tags.append('<span class="job-card-tag job-card-tag-language">🇬🇧 English</span>')

    if employment_type:
        tags.append(
            f'<span class="job-card-tag job-card-tag-employment">{ICONS["briefcase"]} {emp_label}</span>'
        )

    if is_tech_role and technology_category != "other":
        tags.append(
            f'<span class="job-card-tag job-card-tag-category">{tech_svg} {tech_label}</span>'
        )

    if is_tech_role:
        tags.append(
            f'<span class="job-card-tag job-card-tag-tech">{ICONS["tech_role"]} Tech Role</span>'
        )
    else:
        tags.append(
            f'<span class="job-card-tag job-card-tag-nontech">{ICONS["non_tech"]} Non-Tech</span>'
        )
